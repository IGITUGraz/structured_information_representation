...
/* BeginDocumentation
  Name: stdp_synapse_sem - Synapse type for spike-timing dependent
   plasticity with additional additive factors.

  Description:
   stdp_synapse_sem is a connector to create synapses with spike time
   dependent plasticity. Unlike stdp_synapse, we use the update equations:

   \Delta w = lambda * (exp((t - t_j^(k))/tau_+) - A_-)       if t - t_j^(k) > 0
   \Delta w = lambda * A_-                                    if t - t_j^(k) < 0

 This makes it possible to use update rules as in [1].

  Parameters:
   tau_plus   double - Time constant of STDP window, potentiation in ms
                       (tau_minus, which is defined in post-synaptic neuron, is
                       not used)
   lambda     double - Step size
   A_minus    value of depression and offset for facilitation
   Wmax       double - Maximum allowed weight, note: also scales each weight
                       update

  Transmits: SpikeEvent

  References:
   [1] Buesing et. al., Neural Dynamics as Sampling: A Model for Stochastic
       Computation in Recurrent Networks of Spiking Neurons, 2011
...
private:
  ...
  {
    double norm_w = ( w / Wmax_ ) + ( lambda_ * ( kplus - A_minus_ ) );
  ...
  {
    double norm_w = ( w / Wmax_ ) - lambda_ * ( alpha_ * kminus + A_minus_ );
